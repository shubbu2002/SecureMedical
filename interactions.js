...
import { medical, provider } from "./reducer";
...
export const submitRecord=async(name,age,gender,bloodType,allergies,diagnosis,treatment,provider,medical,dispatch)=>{
 ...
 export const deleteData = async(medical,recordId,dispatch,provider)=>{
  let transaction;
  dispatch({
    type : "DELETE_REQUEST_INITIALIZED"
  });
  try {
   const signer = await provider.getSigner();
 ...
 export const loadAllData = async(provider,medical,dispatch)=>{
  ...
  const medicalStream = await medical.queryFilter("MedicalRecord__AddRecord",0,block);
  const medicalRecords = medicalStream.map((event)=>event.args);
  dispatch({type:"ALL_MEDICAL_RECORDS",medicalRecords});
  const deleteStream = await medical.queryFilter("MedicalRecord__DeleteRecord",0,block);
...
export const subscribeToEvent = async(medical,dispatch)=>{
  medical.on("MedicalRecord_AddRecord",(recordId,timestamp,name,age,gender,bloodType,allergies,diagnosis,treatment,event)=>{
    const medicalOrder = event.args;
    dispatch({type:"NEW_RECORD_SUCCESS",medicalOrder,event});
    medical.on("MedicalRecord__DeleteRecord",(recordId,timestamp,name,age,gender,bloodType,allergies,diagnosis,treatment,event)=>{
